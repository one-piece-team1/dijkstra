...
public:
    Dijkstra() : num_vertex(0) {}
    Dijkstra(int n) : num_vertex(n) { AdjList.resize(num_vertex); }
    ~Dijkstra();
    ...
    void Excute(int Start = 0); // 需要Min-Priority Queue
    friend class BinaryHeap;    // 以Binary Heap實現Min-Priority Queue
};

void Dijkstra::Excute(int Start)
{
...
void Dijkstra::InitializeSingleSource(int Start)
{
...
void Dijkstra::Relax(int from, int to, int weight)
{
...
void Dijkstra::PrintDataArray(std::vector<int> array)
{
    for (int i = 0; i < num_vertex; i++)
